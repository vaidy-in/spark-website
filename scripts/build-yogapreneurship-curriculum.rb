...
# Fetch prod chapter metadata for Yogapreneurship sessions and emit curriculum JSON + HTML fragment.
# Usage (from repo root):
#   ruby marketing/scripts/build-yogapreneurship-curriculum.rb
#   ruby marketing/scripts/build-yogapreneurship-curriculum.rb --copy-assets
...
SESSIONS = [
  { number: 1, video_id: '169328', short_title: 'Strategies for Increasing Student Base' },
  { number: 2, video_id: '169724', short_title: 'The 3 Roles' },
  { number: 3, video_id: '170041', short_title: 'What is your niche' },
  { number: 4, video_id: '170626', short_title: 'Strategy and Synergy' },
  { number: 5, video_id: '170899', short_title: 'Tools & Social Media' },
  { number: 6, video_id: '171452', short_title: 'Investment & Pricing' },
  { number: 7, video_id: '171763', short_title: 'Time & Priorities' },
  { number: 8, video_id: '172359', short_title: 'Professionalism for Yoga Teachers' },
  { number: 9, video_id: '172678', short_title: 'Professional Systems and Policies' },
  { number: 10, video_id: '173238', short_title: 'Energy, Credibility, and Consistency' },
  { number: 11, video_id: '173527', short_title: 'Ambition and Holistic Growth' }
].freeze

SLIDE_SAMPLE_SESSIONS = [1, 6, 10].freeze
MAX_SLIDES_PER_SESSION = 4

PROD_ROOT = File.join(ROOT, 'tmp', 'yogapreneurship-prod')
OUT_JSON = File.join(ROOT, 'marketing', 'data', 'yogapreneurship-curriculum.json')
OUT_FRAGMENT = File.join(ROOT, 'marketing', 'data', 'yogapreneurship-curriculum-fragment.html')
OUT_PROGRAM_MAP = File.join(ROOT, 'marketing', 'data', 'yogapreneurship-program-map-fragment.html')
IMG_ROOT = File.join(ROOT, 'marketing', 'images', 'courses', 'yogapreneurship')
...
def format_hours_label(total_seconds)
  return '~20+ hr' if total_seconds <= 0
...
SESSIONS.each do |seed|
  ...
  if SLIDE_SAMPLE_SESSIONS.include?(seed[:number])
    ...
    keys_to_fetch.each_with_index do |s3_key, idx|
      ...
      rel = "../images/courses/yogapreneurship/slides/session-%02d/#{fname}" % seed[:number]
  ...
  if copy_assets
    ...
    if File.file?(thumb_src)
      ...
      thumb_rel = "../images/courses/yogapreneurship/#{File.basename(sess_dest)}"
    end
  end
...
manifest = {
  'courseTitle' => 'Yogapreneurship',
  'courseSubtitle' => 'Build and scale your yoga teaching practice',
  'instructor' => 'Vikas Shenoy',
  'organization' => 'Pancha Yoga',
